...
# Listas de baneados (en memoria por ahora)
banned_users = set()
...
def report():
    ...
    # <<< CAMBIA ESTE NÚMERO POR EL ID DE TU CANAL DE DISCORD >>>
    channel_id = 123456789012345678  # ← PON AQUÍ EL ID REAL
    ...
    embed.set_footer(text="Anti-Executor v2 - Con botones interactivos")
...
async def on_interaction(interaction):
    ...
    if custom_id.startswith("block_"):
        ...
        await interaction.response.send_message(f"🔒 Ejecución bloqueada para UserID `{userId}`", ephemeral=True)
    elif custom_id.startswith("ban_"):
        banned_users.add(userId)
        await interaction.response.send_message(f"🔨 UserID `{userId}` baneado permanentemente", ephemeral=True)
    elif custom_id.startswith("unban_"):
        banned_users.discard(userId)
        blocked_execution.discard(userId)
        await interaction.response.send_message(f"✅ UserID `{userId}` desbaneado", ephemeral=True)
...
if __name__ == "__main__":
    ...
    bot.run(os.getenv("TOKEN"))  # El token se carga desde variable de entorno
